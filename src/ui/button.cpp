...
void Button::AttachShader(Shader *shdr) { shader = shdr; }
...
void Button::AttachTexture(const char *path) { textureId = createTexture(path); }
...
bool Button::IsHovered(double mx, double my) { return mx >= x && mx <= x + width && my >= y && my <= y + height; }

void Button::Activate() { activateCallback(); }
